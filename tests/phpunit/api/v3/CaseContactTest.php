...
class api_v3_CaseContactTest extends CiviCaseTestCase {
  ...
  /**
   * Test Create update function with valid parameters.
   */
  public function testCaseContactCreateUpdate() {
    $params = $this->_params;
    $result = $this->callAPIAndDocument('CaseContact', 'create', $params, __FUNCTION__, __FILE__);
    $params['id'] = $result['id'];
    // Check we can create again with the same parameters
    $result = $this->callAPISuccess('CaseContact', 'create', $params);

    // Check result
    $result = $this->callAPISuccess('CaseContact', 'get', array('id' => $params['id']));
    $this->assertEquals($result['values'][$params['id']]['case_id'], $params['case_id']);
    $this->assertEquals($result['values'][$params['id']]['contact_id'], $params['contact_id']);
  }
